...
export function ContextBar() {
  ...
  if (
    !pathname.startsWith("/app/vendor-supplier") ||
    ...
    <div className="mb-4 rounded-xl border border-slate-200 bg-slate-100 px-3 py-3 shadow-sm">
      ...
      <div className="grid gap-2 md:grid-cols-2">
        <label className="space-y-1">
          <span className="text-[10px] uppercase tracking-[0.1em] text-slate-500">Supplier Ledger</span>
          <select
      ...
      <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2 text-[11px] text-slate-500">
        Active: workspace {selectedWorkspaceName} | supplier ledger {ledgerPda ? `${ledgerPda.slice(0, 4)}...${ledgerPda.slice(-4)}` : "-"} | customer {customerId ?? "-"}
      </div>
    </div>
  );
}
